...
class TestTreatAndGetAny(unittest.TestCase):
    ...
    def test_positive_result_with_expected_type(self):
        ...
        name = PySchema.treat_and_get_any(user_data, 'name',convert=True, expected_type=str)
    ...
    def test_positive_result_with_default_and_convert(self):
        user_data = {}
        name = PySchema.treat_and_get_any(user_data, 'name', default=3, expected_type=str, convert=True)
        self.assertEqual(name, '3')
    
    def test_positive_result_with_default_and_expected_value(self):
        user_data = {}
        name = PySchema.treat_and_get_any(user_data, 'name', default='John', expected_value='John')
        self.assertEqual(name, 'John')
    

    def test_positive_result_with_default_and_inside(self):
        user_data = {}
        name = PySchema.treat_and_get_any(user_data, 'name', default='John', inside=['John'])
        self.assertEqual(name, 'John')
    ...
    def test_positive_result_with_default_and_in_types(self):
        user_data = {}
        name = PySchema.treat_and_get_any(user_data, 'name', default='John', in_types=[str])
        self.assertEqual(name, 'John')
    

    def test_positive_result_with_default_and_convert(self):
        user_data = {}
        name = PySchema.treat_and_get_any(user_data, 'name', default='3', expected_type=int, convert=True)
        self.assertEqual(name, 3)
    ...
    def test_negative_result_with_convert(self):
        ...
        with self.assertRaises(PySchema.PySchemaException):
            PySchema.treat_and_get_any(user_data, 'name', expected_type=int, convert=False)
    

    def test_negative_result_with_default(self):
        user_data = {}
        with self.assertRaises(PySchema.PySchemaException):
            PySchema.treat_and_get_any(user_data, 'name')
